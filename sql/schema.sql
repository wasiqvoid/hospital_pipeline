...
-- BILLING (includes patient_id from CSV; payment_status includes Failed)
CREATE TABLE billing (
    bill_id        VARCHAR(10)   PRIMARY KEY,
    patient_id     VARCHAR(10)   REFERENCES patients(patient_id)    ON DELETE SET NULL,
    treatment_id   VARCHAR(10)   NOT NULL REFERENCES treatments(treatment_id) ON DELETE CASCADE,
    bill_date      DATE          NOT NULL,
    amount         DECIMAL(10,2) NOT NULL CHECK (amount >= 0),
    payment_method VARCHAR(50),
    payment_status VARCHAR(20)   NOT NULL DEFAULT 'Pending'
                   CHECK (payment_status IN ('Pending','Paid','Failed','Partial','Waived')),
    created_at     TIMESTAMPTZ   NOT NULL DEFAULT NOW()
);
